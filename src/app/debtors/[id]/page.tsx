...
export default function DebtorDetailPage() {
  ...
  const [debtor, setDebtor] = useState(debtors.find((d) => d.id === id));
  ...
  useEffect(() => {
    const found = debtors.find((d) => d.id === id);
    if (found) {
      setDebtor(found);
    } else {
      router.push('/debtors');
  ...
  const totalAmount = -debtor.transactions.reduce((acc, curr) => acc + curr.amount, 0);
